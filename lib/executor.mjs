...
export async function runTask(taskFile, logFile) {
  ...
  try {
    ...
    // since Claude CLI may return non-zero even when the task completes successfully
    const result = await $`cd ${repoCodePath} && echo ${taskContent} | ${claudePath} -p "Execute this task" --verbose --output-format text --dangerously-skip-permissions > ${absoluteLogFile} 2>&1`.nothrow();

    if (result.exitCode !== 0) {
      console.log(`⚠️  Claude CLI exited with code ${result.exitCode} for ${taskName} (check execution.log for details)`);
    }
